...
accounting_data_length = {ENTITY_TYPE: 2, TRANSACTION_TYPE: 8, ACCOUNT_TYPE: 5}
...
def get_transaction_dict_from_data(data):
    if len(data) != accounting_data_length.get(TRANSACTION_TYPE):
        return None
    initiator_entity, destinatary_entity, reference_accounts, initiator_account, destinatary_account, transfer_ratio, \
    minimum_transfer, maximum_transfer = data[:]
    kargs = dict()
    if reference_accounts is not None:
        kargs["reference_accounts"] = set([val.strip() for val in reference_accounts.split(";")])
    ...
    transaction = transactionModule.get_transaction_dict(initiator_account, destinatary_account, transfer_ratio,
                                                         **kargs)
    return {"initiator_entity": initiator_entity, "destinatary_entity": destinatary_entity, "transaction": transaction}
...
entities_dict_list, accounts_dict_list, transaction_dict_list = read_accounting_book('accounting_sheet.xlsx')

entities_network = entitiesNetworkModule.create_entities_network(entities_dict_list,
                                                                 accounts_dict_list, transaction_dict_list)
